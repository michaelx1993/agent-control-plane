...
if [[ "${OPERATOR_QUERY_SMOKE_TEMP_DB:-true}" != "false" ]]; then
  TEMP_DATABASE_NAME="acp_operator_query_smoke_$(date +%s)_$$"
  TEMP_DATABASE_URL="$(
    node -e 'const url = new URL(process.argv[1]); url.pathname = "/" + process.argv[2]; process.stdout.write(url.toString());' \
      "$SMOKE_DATABASE_URL" \
      "$TEMP_DATABASE_NAME"
  )"
...
DATABASE_URL="$SMOKE_DATABASE_URL" pnpm --silent --filter @agent-control-plane/db operator:query-smoke
