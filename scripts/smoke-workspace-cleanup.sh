...
if [[ "${WORKSPACE_CLEANUP_SMOKE_TEMP_DB:-true}" != "false" ]]; then
  TEMP_DATABASE_NAME="acp_workspace_cleanup_smoke_$(date +%s)_$$"
...
DATABASE_URL="$SMOKE_DATABASE_URL" pnpm --silent --filter @agent-control-plane/worker workspace:cleanup-smoke
