...
if [[ "${WORKER_FAIRNESS_SMOKE_TEMP_DB:-true}" != "false" ]]; then
  TEMP_DATABASE_NAME="acp_fairness_smoke_$(date +%s)_$$"
...
DATABASE_URL="$SMOKE_DATABASE_URL" pnpm --silent --filter @agent-control-plane/worker worker:fairness-smoke
