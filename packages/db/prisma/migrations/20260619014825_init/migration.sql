-- CreateEnum
CREATE TYPE "ExternalProvider" AS ENUM ('plane', 'github', 'local');
...
-- CreateEnum
CREATE TYPE "TaskState" AS ENUM ('Todo', 'Development', 'Code Review', 'Human Review', 'In Merge', 'Merged', 'Release Version', 'Released', 'Deployment', 'Deployed', 'Done', 'Blocked', 'Canceled');

-- CreateEnum
CREATE TYPE "AgentRuntime" AS ENUM ('openhands');
...
-- CreateEnum
CREATE TYPE "PromptScopeType" AS ENUM ('global', 'team', 'project', 'repo', 'role', 'agent');

-- CreateEnum
CREATE TYPE "PromptComponentStatus" AS ENUM ('draft', 'active', 'archived');

-- CreateEnum
CREATE TYPE "PromptBindingEnvironment" AS ENUM ('dev', 'staging', 'prod');

-- CreateEnum
CREATE TYPE "PromptBindingStatus" AS ENUM ('active', 'disabled');

-- CreateEnum
CREATE TYPE "RunStatus" AS ENUM ('queued', 'claimed', 'running', 'succeeded', 'blocked', 'failed', 'canceled');

-- CreateEnum
CREATE TYPE "WorkspaceStrategy" AS ENUM ('clone', 'worktree', 'existing');

-- CreateEnum
CREATE TYPE "WorkspaceStatus" AS ENUM ('preparing', 'ready', 'dirty', 'archived', 'cleaned');

-- CreateEnum
CREATE TYPE "ConversationProvider" AS ENUM ('openhands');

-- CreateEnum
CREATE TYPE "TraceProvider" AS ENUM ('langfuse');

-- CreateEnum
CREATE TYPE "RunEventType" AS ENUM ('queued', 'claimed', 'heartbeat', 'state_sync', 'blocked', 'completed', 'failed', 'canceled');

-- CreateEnum
CREATE TYPE "FeedbackSource" AS ENUM ('human', 'code_review', 'pr_review', 'agent', 'plane_comment');

-- CreateEnum
CREATE TYPE "FeedbackSeverity" AS ENUM ('info', 'minor', 'major', 'blocker');
...
CREATE TABLE "agent_definitions" (
    ...
    "runtime" "AgentRuntime" NOT NULL DEFAULT 'openhands',
    "model" TEXT NOT NULL,
    ...
    "status" "AgentDefinitionStatus" NOT NULL DEFAULT 'active',
    "created_at" TIMESTAMPTZ(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updated_at" TIMESTAMPTZ(6) NOT NULL,
...
CREATE TABLE "prompt_components" (
    "id" UUID NOT NULL,
    "scope_type" "PromptScopeType" NOT NULL,
    "scope_id" UUID,
    "name" TEXT NOT NULL,
    "version" INTEGER NOT NULL,
    "status" "PromptComponentStatus" NOT NULL DEFAULT 'draft',
...
CREATE TABLE "prompt_bindings" (
    ...
    "environment" "PromptBindingEnvironment" NOT NULL DEFAULT 'dev',
    "status" "PromptBindingStatus" NOT NULL DEFAULT 'active',
    "created_at" TIMESTAMPTZ(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updated_at" TIMESTAMPTZ(6) NOT NULL,
...
CREATE TABLE "workspaces" (
    ...
    "repository_id" UUID NOT NULL,
    "strategy" "WorkspaceStrategy" NOT NULL,
    "path" TEXT NOT NULL,
    "base_ref" TEXT NOT NULL,
    "head_ref" TEXT,
    "status" "WorkspaceStatus" NOT NULL DEFAULT 'preparing',
    "created_at" TIMESTAMPTZ(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE "conversation_refs" (
    "id" UUID NOT NULL,
    "run_id" UUID NOT NULL,
    "provider" "ConversationProvider" NOT NULL DEFAULT 'openhands',
...
CREATE TABLE "trace_refs" (
    "id" UUID NOT NULL,
    "run_id" UUID NOT NULL,
    "provider" "TraceProvider" NOT NULL DEFAULT 'langfuse',
...
-- CreateIndex
CREATE INDEX "prompt_bindings_scope_type_scope_id_environment_status_idx" ON "prompt_bindings"("scope_type", "scope_id", "environment", "status");

-- CreateIndex
CREATE INDEX "prompt_releases_task_id_idx" ON "prompt_releases"("task_id");

-- CreateIndex
CREATE INDEX "prompt_releases_repository_id_idx" ON "prompt_releases"("repository_id");
...
-- CreateIndex
CREATE INDEX "runs_task_id_created_at_idx" ON "runs"("task_id", "created_at" DESC);
...
-- AddForeignKey
ALTER TABLE "prompt_releases" ADD CONSTRAINT "prompt_releases_task_id_fkey" FOREIGN KEY ("task_id") REFERENCES "tasks"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE "runs" ADD CONSTRAINT "runs_task_id_fkey" FOREIGN KEY ("task_id") REFERENCES "tasks"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE "runs" ADD CONSTRAINT "runs_prompt_release_id_fkey" FOREIGN KEY ("prompt_release_id") REFERENCES "prompt_releases"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE "trace_refs" ADD CONSTRAINT "trace_refs_prompt_release_id_fkey" FOREIGN KEY ("prompt_release_id") REFERENCES "prompt_releases"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
