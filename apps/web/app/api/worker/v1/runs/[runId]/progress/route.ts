...
import { recordTaskProgress, withDatabasePool, withTransaction } from "@agent-control-plane/db";
import {
  executeWorkerWrite,
  isRouteFailure,
  optionalString,
  ...
  const result = await withDatabasePool((pool) =>
    withTransaction(pool, (client) =>
      executeWorkerWrite(client, safety, async () => {
        ...
        const externalUrl = optionalString(payload, "externalUrl");
        const progress = await recordTaskProgress(client, {
          taskId: lease.taskId,
          runId,
          body,
