...
- 已有 OpenHands adapter、workspace 记录、可配置 runtime endpoint paths、conversation refs、poll heartbeat hook。
...
- `pnpm live:dispatch-once` 会先执行 live preflight，再派发一个真实任务用于 Development run smoke test。
