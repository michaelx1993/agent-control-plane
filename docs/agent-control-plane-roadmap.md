...
截至 2026-06-26，当前 `aiworkspace` / `agent-control-plane` / `agent-worker` / `plane` 已完成默认 `codex-cli` profile 的产品化闭环。P0/P0.5 已完成，P1/P2/P3/P4/P5/P6/P7/P8 的第一版可交付链路均有本地或生产证据：真实 Plane task、Codex run events、Progress/Workpad、Plane writeback、task-source、secret provider、provider audit、production smoke、external preflight 和最终 cutover report 已在 self-host 环境通过 `pnpm completion:final` 审计。Langfuse/OpenHands 仍保留为 optional/legacy profile，不进入第一版阻断条件。后续主线从“证明能替代 Symphony/Linear 执行链”转为“增强产品体验、补历史 Linear 任务迁移、校准长任务和 app-server 长会话”。
...
- 按产品闭环口径，默认 `codex-cli` profile 已可替代旧 Symphony/Linear agent 任务源：真实 Plane task、Control Plane run evidence、Plane writeback、task-source、secret provider、provider audit、legacy poller freeze、Linear archive、worker crash/budget/workflow 和 final cutover report 都已通过审计。
- 最新最终审计：`completion_final=passed`，final run `final-20260626T005616Z-95644`，report `reports/cutover-final-20260626T005616Z-95644.json`，`readiness=passed`，`errors=[]`，`warnings=[]`。
- 下一步主线是产品增强：Plane 上的 Agent/Prompt/Project/Worker 使用体验、历史 Linear 未完成任务的按需导入、真实长任务稳定性、`codex-app-server` 长会话复测和更细粒度 prompt/secret 可见性。
...
| 领域           | 当前状态                                                                                                                                                                                                               | 已验证方式                                                                                                                            | 未完成验收                                                                  |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| Plane 任务层   | 本地 API/webhook/label fallback/回写骨架和 Plane writeback smoke harness 完成；真实 P1-1 writeback 已写 state/comment 并回读验证                                                                                       | Plane PAT/API smoke、webhook smoke、本地单元测试、mock Plane writeback smoke、真实 Plane writeback smoke                              | rework 仍保留 contract smoke，后续可补 UI 级复测                            |
| Runtime/Worker | claim、lease、heartbeat、续租、retry/backoff、stalled、lease 过期后重新派发、Task Queue 筛选、repo/role/agent 并发门禁、基础 priority 排序和单 run 成本门禁完成；真实 Plane-routed `codex-cli` run 已进入最终 evidence | DB 单元测试、本地 mock worker 状态推进、`pnpm worker:lease-smoke`、`pnpm worker:crash-smoke`、真实 task-source/final cutover evidence | 真实超长任务和 workspace 残留恢复仍可继续压测                               |
| Prompt 平台    | component、binding、approval、release、rollback、audit 完成                                                                                                                                                            | 单元测试、Prompt Manager/API smoke                                                                                                    | prompt 变更质量评估和更细粒度回滚体验                                       |
| Codex 执行器   | Symphony 执行模型已沉淀，Worker adapter 抽象、workspace manager、第一阶段 `codex exec --json` adapter 和第一阶段 `codex-app-server` adapter 已具备                                                                     | 设计文档、本地 workspace/worker smoke、真实 Plane task 端到端、Codex-first final cutover                                              | 真实 app-server 长会话复用作为后续增强                                      |
...
| Secret/部署    | validate、rotate、expiry warning、rotation audit log、external command、provider smoke、provider audit file/command smoke、external preflight、completion gap report、deploy/smoke/cutover/completion audit 脚本完成   | 本地脚本 smoke、失败分支验证、真实 secret provider/provider audit/final cutover report                                                | 后续可接真实供应商 API audit command，当前 file evidence 已通过默认 cutover |
| Linear 迁移    | export -> Plane migration CLI 和 cutover gate 完成；当前 ACP `linear_url_tasks=0` 且 Linear archive-only/read-only 已确认                                                                                              | dry-run/缺参失败分支验证、final cutover evidence                                                                                      | 历史未完成 Linear 任务导入可按需单独执行                                    |

当前“开发完成”的口径已经从本地 mock 扩展到默认 `codex-cli` profile 的生产 cutover evidence。当前“产品闭环完成”的口径已由 2026-06-26 final report 满足：

1. 真实 Plane task 由 Worker 使用 `codex-cli` adapter 跑通 Development run。
2. Control Plane evidence 包含 Codex run events、Progress、Workpad、prompt release、workspace 和 summary。
3. Plane state/comment writeback 在 self-host 环境复测通过；rework contract smoke 保持覆盖。
4. `pnpm smoke:production` 在目标部署环境通过，且开启外部只读探针。
...
7. `pnpm completion:final` 在 `codex-cli` profile 下通过，并保留 cutover report `reports/cutover-final-20260626T005616Z-95644.json`。
...
- 部署层：Dockerfile、Compose、CI release gate、镜像发布脚本、Compose 部署脚本、应用镜像回滚脚本、数据库备份/恢复脚本、首页监控 dashboard 最小指标、基础告警、阈值 UI/API 动态配置、generic/slack/email webhook 告警通知、告警失败重放队列、secret validate gate、`secrets:env-smoke`、secret env file/rotation/expiry warning 最小链路、secret rotation audit log、外部 secret command 接入、secret provider smoke、provider audit file/command smoke、production smoke harness、可选外部依赖只读探针、`plane:live-smoke`、cutover gate 和趋势图已完成；2026-06-26 真实端到端外部 smoke、真实 secret provider command 和 provider audit file evidence 已通过 final cutover gate。
- 本地验收层：2026-06-20 已通过 `ACP_LOCAL_COMPLETION_RUN_WEB_BUILD=false pnpm completion:local-smoke`，该命令聚合 `git diff --check`、脚本语法检查、`pnpm check`、`pnpm db:validate`、`pnpm secrets:validate`、Core/DB/Plane build、`operator:query-smoke`、`linear:migrate-smoke`、`plane:human-gate-writeback-smoke`、`worker:contract-smoke`、`worker:codex-plane-smoke` 默认安全 skip、`WORKER_EXECUTION_ADAPTER=codex-app-server pnpm worker:codex-plane-smoke` 安全 skip、`WORKER_EXECUTION_ADAPTER=codex-app-server WORKER_CODEX_PLANE_SMOKE_FOLLOW_UP=true pnpm worker:codex-plane-smoke` 安全 skip、`worker:codex-plane-report-smoke`、`openhands:payload-contract`、`task-source:local-smoke`、`cutover:report-smoke`、`external:preflight-smoke`、`completion:doctor-smoke`、`completion:gap-smoke`、`completion:final-env-template-smoke`、`completion:audit-smoke`、`completion:final-smoke`、`completion:local-web-build-smoke`；`operator:query-smoke` 会在临时 PostgreSQL 数据库上执行真实 operator task/run 查询，覆盖 enum/text 兼容问题；`worker:contract-smoke` 会验证 Control Plane Worker API OpenAPI 文档、run write route handlers 和 idempotency 参数一致；`codex:app-server-smoke` 会用 fake app-server 验证 Codex app-server JSON-RPC 生命周期和事件落库前摘要转换；`worker:codex-plane-report-smoke` 会离线验证真实 Plane smoke JSON evidence report 的字段、`0600` 权限和默认拒绝覆盖；Web production build 默认 auto：无 Next dev lock 时自动纳入验收；有 lock 时跳过并输出持锁 PID；瞬时 Next build lock retry 行为已有独立 smoke 覆盖。
...
- `pnpm worker:codex-plane-smoke` 已作为显式 opt-in 入口落地。拆仓后真实 apply 模式委托 `michaelx1993/agent-worker` 通过 Control Plane Worker API claim/run/complete，验证已有 Plane-routed task 的 Control Plane run、`codex.*` events、git-worktree、任务级 Running / Agent Events / Completed Progress / Workpad 和状态推进；默认安全 skip，真实 apply 要求 `WORKER_CODEX_PLANE_SMOKE_APPLY=true`、`WORKER_CODEX_PLANE_SMOKE_TEMP_DB=false`、`CONTROL_PLANE_BASE_URL`、`ACP_WORKER_API_TOKEN` 和可派发 task。设置 `WORKER_CODEX_PLANE_SMOKE_REPORT_FILE` 时会写出权限为 `0600` 的 JSON evidence report；`pnpm worker:codex-plane-report-smoke` 已离线验证 report 字段、权限和默认拒绝覆盖。设置 `WORKER_CODEX_PLANE_SMOKE_FOLLOW_UP=true` 时不再在脚本内模拟 review 打回，而是要求真实 app-server follow-up run 产生 `codex.thread_reused` event 和 `codex-app-server` conversation ref。
