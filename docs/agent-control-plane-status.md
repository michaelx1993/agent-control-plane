...
当前已经完成 Codex-first 生产 cutover 的可审计闭环：任务镜像、状态机、prompt release、worker claim/lease/retry、workspace 准备、任务级 Progress / Workpad、PR review feedback 摄取、`codex-cli` ExecutionAdapter、OpenHands 可选 adapter 骨架、Langfuse 可选 trace 骨架、Plane writeback、生产 smoke、task-source smoke、secret provider smoke、provider audit smoke、cutover gate、监控、基础权限和最终完成审计。

当前可按默认 `codex-cli` profile 判定为生产完成：`pnpm completion:final` 已在 self-host Plane / ACP / Mac Studio Worker 环境生成真实 cutover report 并通过 `completion:audit`。方案继续保持省钱优先：默认不依赖 OpenHands Cloud、Langfuse Cloud 或任何需要额外付费的 SaaS/Enterprise license。OpenHands/Langfuse 只作为 legacy/optional profile，不阻断第一版。

- `codex-cli` / `codex app-server` ExecutionAdapter：第一阶段已能在目标 repo workspace 中执行 `codex exec --json`，注入 prompt release 和 task context，消费 JSONL 输出；`codex-app-server` adapter 第一阶段已支持 `initialize -> thread/start -> turn/start -> turn/completed` 的 stdio JSON-RPC 生命周期，并复用 Codex 事件归一化/脱敏；Worker 会把同一 task 的上一条同 provider `conversation_ref` 注入下一次执行，`codex-app-server` adapter 能从上一条 ref 恢复 thread id 并直接启动新 turn；Worker loop 会在进程内复用同一个 execution adapter 实例，`codex-app-server` 默认开启 persistent session，让多轮 turn 共用同一个 app-server 进程，`WORKER_CODEX_APP_SERVER_PERSISTENT=false` 可退回每 turn 新进程；`worker:codex-smoke` 已验证 Worker 可用 fake Codex 走通 claim、git-worktree workspace、prompt release、`run_events`、`conversation_refs`、Progress 事件摘要和 Development -> Code Review；`worker:codex-real-smoke` 已用本机真实 Codex 分别跑通 `codex-cli` 单 turn、`codex-app-server` 单 turn，以及 `codex-app-server` follow-up 两轮同 task thread reuse；self-host P1-1 已用真实 Plane task 跑通 `codex-cli` Development run，并被最终 task-source smoke 纳入审计。
...
- 真实 Plane project 上的状态/comment writeback 已通过最终 Plane writeback smoke；rework contract 仍由本地 `plane:human-gate-writeback-smoke` 覆盖。
- 真实 production smoke 的外部依赖只读探针已在最终 `completion:final` 中通过。
- 真实 task-source 样本已证明新任务只从 Plane/Control Plane 派发：`checked=1;plane_urls=1;linear_urls=0;routed=1;runs=1;run_events=1;progress_items=1;prompt_releases=1;workspaces=1`。
- 真实 secret provider command smoke 已通过：`variables=28;validation=passed`。
- provider audit smoke 已通过：`source=file;events=1;matched_events=1;newest_event_at=2026-06-26T00:55:00.000Z`。
- 基于上述证据生成的 `reports/cutover-final-20260626T005616Z-95644.json` 已通过 `completion:final` / `completion:audit`。

当前 `pnpm cutover:rehearsal` 继续作为本地 mock 外部服务的一键演练，用来验证 cutover gate 和 smoke harness 的编排；生产批准信号以 `completion:final` 生成的真实 cutover report 为准。旧 Symphony poller 已停用/只读，Linear 已归档只读，ACP 当前 `linear_url_tasks=0`。若后续仍需迁移历史 Linear 未完成任务，应单独执行 Linear export -> Plane import，不再作为当前 Codex-first cutover 的完成阻断项。

## 最近一次生产完成审计

2026-06-26 最终验收：

- `pnpm completion:final` 通过，`completion_final=passed`，final run `final-20260626T005616Z-95644`。
- cutover report：`reports/cutover-final-20260626T005616Z-95644.json`，`reportId=cutover-report-final-20260626T005616Z-95644`，`readiness=passed`，`errors=[]`，`warnings=[]`。
- profile：`ACP_COMPLETION_EXECUTION_PROFILE=codex-cli`，`WORKER_EXECUTION_ADAPTER=codex-cli`。
- production smoke、Plane writeback、Codex adapter smoke、task-source、worker crash recovery、worker budget、worker workflow、secret provider、provider audit、external preflight 全部为 true。
- Plane writeback evidence：`work_item_id=766de24a-5ed9-4314-9604-40338331af64;state=In Progress;comment=created;verified=true`。
- task-source evidence：`checked=1;plane_urls=1;linear_urls=0;routed=1;runs=1;run_events=1;progress_items=1;prompt_releases=1;workspaces=1`。
- external preflight evidence：`preflight_id=external-preflight-final-20260626T005616Z-95644;ready_count=7;missing_count=0`。
- cutover evidence：legacy poller disabled/read-only，Linear archive-only/read-only，ACP `linear_url_tasks=0`。
...
- 2026-06-26 已用真实 Plane task 跑通 `codex-cli` Development run，并让最终门禁以真实 Codex run evidence、Progress/Workpad、Plane writeback 和 task-source evidence 通过审计；下一步转向产品体验和长任务稳定性增强。
...
- 2026-06-25 Plane projection -> runtime routing mirror 增量：`agent_project_workspace` projection 写入后会自动物化 legacy `projects`，`agent_repository` projection 写入后会自动物化 legacy `repositories`；若 repository projection 先于 project projection 到达，project projection 物化完成后会补偿物化该 project 下已有 repository projections。这样 Plane run intent 不再需要手工补 `projects/repositories` 映射才能通过 `upsertPlaneRunIntentTask` 路由。该增量已通过 `pnpm --filter @agent-control-plane/db test -- plane-agent-runtime.test.ts task-sync.test.ts`、`pnpm --filter @agent-control-plane/db typecheck`、`pnpm format`、`pnpm db:validate`，并在临时 PostgreSQL 上验证 repo-before-project 乱序 projection 能最终生成 legacy project/repository mirror。
- 2026-06-25 Seed demo / task-source 门禁修正：静态 seed demo `TOK-1` / `demo-task-1` 不再默认处于自动派发状态，新 migration 会把已部署数据库中无 Plane URL 的该 demo task 移到 `Human Review`。`task-source:smoke` 默认会审计自动节点任务，也会纳入已经推进到 Human Review 但仍有 latest run evidence 的任务；真实 cutover smoke 应用 `TASK_SOURCE_SMOKE_PROJECT_SLUG=<real-plane-project-slug>` 限定到真实 Plane project，避免本地 fixture 干扰生产任务来源审计。
...
- 2026-06-25 Run Detail Runtime Snapshot 可见性增量：ACP operator Run Detail 已展示 Plane runtime snapshot id/hash/schema、prompt stack、assembled prompt preview 和 available secret keys；secret 仍只显示 key 名，不展示 value。该增量补齐 PRD 中 “Run detail 必须展示 assembled prompt / 来源版本 / available secret keys” 的最小可见性要求。
- 2026-06-25 Task Run Preview 发布部署：ACP PR #47 已合并到 `main`，发布 tag `agent-control-plane-v0.0.12`，GitHub Release run `28180011661` 成功发布 `michaelxxx/agent-control-plane:0.0.12`。Mac Studio/self-host ACP 已升级到 `0.0.12`，revision label `c1c92312b6f0890c1f31bf939429f242ebe20033`；部署前备份：`/Users/a/agent-control-plane/backups/agent-control-plane-pre-0.0.12-20260625T151638Z.dump`。验收通过：`pnpm smoke:production` passed，ACP readiness ready，生产 `Development` task `TOK-1` 的 `GET /api/tasks/[taskId]/run-preview` 返回 200，Task Detail 页面包含 Run Preview、Prompt stack、Secret keys 和 Assembled prompt preview。
- 2026-06-25 Project Meta Git 真实验收：Agent Worker PR #17 已合并，发布 tag `agent-worker-v0.0.3`，GitHub Release run `28181287168` 成功发布 `ghcr.io/michaelx1993/agent-worker:0.0.3`。Mac Studio LaunchAgent 已基于 `michaelx1993/agent-worker` main `049b570dc4401b15527804ed2d580f660d426cd9` 重新 `pnpm build` 并重启。生产 `TOK-1` 真实 run `781c7239-0669-4835-be61-25658fe440ed` 已完成 Development -> Code Review，Worker artifacts payload 包含 `projectMetaGit`，本地 Meta Git repo `/Users/a/agent-worker-workspaces/_project-meta/token` 产生 commit `6012daa884caa328d2d2533822df14591e719bfe`，ACP 已记录 1 个 `acp_project_meta_repos` 和 4 条 `acp_project_memory_commits`。
...
- 2026-06-25 Plane/ACP PRD 对齐发布部署：Plane PR #18 已合并到 `preview`，发布 tag `plane-components-v0.0.8`，GitHub release-components run `28175976843` 成功发布 `michaelxxx/plane-{frontend,backend,admin,space,live,proxy}:0.0.8`；ACP PR #43 已合并到 `main`，发布 tag `agent-control-plane-v0.0.10`，GitHub release run `28175976141` 成功发布 `michaelxxx/agent-control-plane:0.0.10`。Mac Studio/self-host 部署已升级：Plane migration `db.0123_agent_platform_prd_alignment` 已应用，ACP migration `0013_user_secret_key_projection` 已应用。部署前备份：`/Users/a/plane-selfhost/backups/plane-pre-0.0.8-20260625T142631Z.dump`、`/Users/a/agent-control-plane/backups/agent-control-plane-pre-0.0.10-20260625T141324Z.dump`。验收通过：Plane 本机/公网入口 200，ACP readiness ready，`pnpm smoke:production` passed，`pnpm plane:agent-config-sync` passed。Mac Studio worker LaunchAgent running，队列当前为空，stderr 最后写入早于本轮部署。
- 2026-06-25 Run Detail Runtime Snapshot 发布部署：ACP PR #45 已合并到 `main`，发布 tag `agent-control-plane-v0.0.11`，GitHub Release run `28178294873` 成功发布 `michaelxxx/agent-control-plane:0.0.11`。Mac Studio/self-host ACP 已升级到 `0.0.11`，revision label `b4cb0fc26cdecc9471b16107714dfe9c371cd808`；部署前备份：`/Users/a/agent-control-plane/backups/agent-control-plane-pre-0.0.11-20260625T145023Z.dump`。验收通过：`pnpm smoke:production` passed，ACP readiness ready，生产 Run Detail 页面验证包含 Runtime Snapshot、Prompt stack、Secret keys 和 Assembled prompt preview。
...
- 这次 2026-06-20 的历史本地验收只证明本地 harness、单元测试、构建、真实 Codex 本机 smoke 和 mock cutover 编排可闭环；它不是生产完成证据，也不能替代真实 Plane task、Plane writeback、production smoke、task-source、secret provider、provider audit 和最终 cutover report。2026-06-26 已由 `completion:final` 补齐真实生产 cutover evidence。OpenHands/Langfuse 只在 optional/legacy profile 下另行验收。
...
| 真实外部预检      | `ACP_SECRET_ENV_FILE=<file> pnpm external:preflight` 输出 `external_smoke_preflight=passed` 且 `missing_count=0`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 已通过：`external-preflight-final-20260626T005616Z-95644`，`ready_count=7;missing_count=0` |
| 真实 cutover gate | `pnpm cutover:check` 通过并保存 `ACP_CUTOVER_REPORT_FILE`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 已通过：`reports/cutover-final-20260626T005616Z-95644.json`，`readiness=passed`            |
| 最终完成审计      | `pnpm completion:final` 基于真实外部配置生成并审计 cutover report，且未开启任何 allow/skip 调试开关                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 已通过：`completion_final=passed`，final run `final-20260626T005616Z-95644`                |
...
- 已用真实 Plane-routed task / split HTTP Worker / 宿主 Codex 跑通一次 Development run `f8ead3c4-79c2-41bb-b086-dfe18e640601`。仍需用更多真实业务长任务校准 `run_events` / Progress / Workpad / Run Detail 的事件分类、截断策略和噪声过滤；`codex-app-server` previous thread resume、Worker loop adapter 复用和 persistent app-server follow-up 已有 fake 与本机真实 Codex 临时 DB 验证，仍未证明真实 Plane 派发路径下的跨 turn 长会话复用。
...
- 已完成 `0012_project_projection_status` 与 `0013_user_secret_key_projection` 的真实部署迁移；完整 cutover 演练仍未完成。
