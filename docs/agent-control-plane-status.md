...
- 2026-06-25 Task Run Preview 本地增量：ACP Task Detail 已新增非持久化 Run Preview，自动执行状态的任务可在启动前预览 role、agent、worker、prompt stack、available secret keys 和 assembled prompt；同时新增 `GET /api/tasks/[taskId]/run-preview`，供 Plane Run Create / 任务分配 UI 后续接入。该增量已通过 `pnpm format`、`git diff --check`、`pnpm test`、`pnpm typecheck` 和 `pnpm build`，尚未发布部署。
