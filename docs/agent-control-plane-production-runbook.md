...
- `pnpm worker:codex-plane-smoke` 会真实调用 Plane API 并要求 `WORKER_CODEX_PLANE_SMOKE_APPLY=true`、`PLANE_BASE_URL`、`PLANE_WORKSPACE_SLUG`、`PLANE_PROJECT_ID`、`PLANE_API_KEY`；默认创建临时 DB 和 Plane work item，验证 Codex worker claim、Control Plane run、git-worktree workspace、`codex.*` events、任务级 Running / Agent Events / Completed Progress / Workpad、状态推进和 Plane comment/state 写回。默认使用 `codex-cli`，也可显式设置 `WORKER_EXECUTION_ADAPTER=codex-app-server`。若同时设置 `WORKER_CODEX_PLANE_SMOKE_FOLLOW_UP=true`，脚本会要求 app-server adapter，第一轮写回 Code Review 后模拟 review 打回 Development，再跑第二轮并验证第二轮 Progress / Workpad、`codex.thread_reused`、同 thread id 和第二次 Plane writeback。
...
- 设置 `ACP_CUTOVER_RUN_TASK_SOURCE_SMOKE=true` 后，cutover gate 会执行 `scripts/smoke-task-source.sh`，审计非终态自动派发任务的 URL 来源、repo routing 和 run evidence。默认 `codex-cli` profile 应以 Control Plane run events / Progress / Workpad 证明执行，conversation/trace evidence 只属于 legacy external profile。
...
task_source_evidence=<checked/routed/run/conversation/trace counts>
